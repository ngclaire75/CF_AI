...
# Stage 2: openssl s_client — different TLS implementation, bypasses source-IP filters that only target HTTP clients
openssl s_client -connect {{domain}}:443 -servername {{domain}} -brief </dev/null 2>&1 | grep -iE "(subject|issuer|notbefore|notafter|subjectAltName|san)" | head -15 || echo "[openssl:443] no cert"
# Stage 3: Alternative SSL ports — exploits overly broad rules (admins open 8443/4443 without IP restriction)
openssl s_client -connect {{domain}}:8443 -servername {{domain}} -brief </dev/null 2>&1 | grep -iE "(subject|issuer|notafter|san)" | head -10 || echo "[openssl:8443] no SSL"
openssl s_client -connect {{domain}}:4443 -servername {{domain}} -brief </dev/null 2>&1 | grep -iE "(subject|issuer|notafter|san)" | head -10 || echo "[openssl:4443] no SSL"
...
# Stage 7: SMTP/IMAP/POP3 STARTTLS — mail server certs often share the same certificate as the web server
# Exploits: exposed management ports (item 7), mail services often on different IP or with looser rules
python3 -c "import subprocess; r=subprocess.run(['openssl','s_client','-connect','{{domain}}:587','-starttls','smtp','-brief'],input=b'',capture_output=True,timeout=12); out=r.stdout.decode(errors='replace'); [print('[SMTP STARTTLS]',l) for l in out.splitlines() if any(k in l.lower() for k in ('subject','issuer','notafter','san'))][:10] or print('[SMTP:587] no TLS cert'); r2=subprocess.run(['openssl','s_client','-connect','{{domain}}:993','-brief'],input=b'',capture_output=True,timeout=12); out2=r2.stdout.decode(errors='replace'); [print('[IMAPS:993]',l) for l in out2.splitlines() if any(k in l.lower() for k in ('subject','issuer','notafter','san'))][:10] or print('[IMAPS:993] no cert')"
