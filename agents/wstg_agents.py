...
python3 -c "import subprocess,json; domain='{{domain}}'; raw=subprocess.run(['curl','-4','-sk','--max-time','12','--connect-timeout','8','-A','curl/7.88','https://archive.org/wayback/available?url='+domain],capture_output=True,text=True,timeout=15).stdout.strip(); d=json.loads(raw) if raw.startswith('{') else {{}}; snap=d.get('archived_snapshots',{{}}).get('closest',{{}}).get('url',''); print('Wayback snapshot:',snap) if snap else print('[Wayback snapshot] NONE —', '[REASON] Empty/non-JSON API response. archive.org may be unreachable from this VPS or rate-limiting. Raw response: '+raw[:200] if not d else '[REASON] Wayback has never archived this domain. Next steps: (1) check archive.org/web manually from browser, (2) use Google cache: webcache.googleusercontent.com/search?q=cache:'+domain+', (3) try Common Crawl via commoncrawl.org/the-data/get-started')"
